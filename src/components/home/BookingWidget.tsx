...
export default function BookingWidget() {
  ...
  const [tripType, setTripType] = useState<"one-way" | "round-way" | "hourly">("one-way");

  return (
    <div className="layout-container relative z-10 -mb-28 sm:-mb-32">
      {/* Top Tabs */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 pb-6 border-b border-gray-100">
          ...
          <div className="flex flex-col gap-1.5 p-3 rounded-xl border border-gray-200 hover:border-blue-primary transition-colors">
            <label className="text-xs font-semibold text-gray-800 flex items-center gap-1.5">
              <span>🚗</span> Choose a Car <span className="text-red-500">*</span>
            </label>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0">
            {[
              ...
              <button
                ...
                className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all cursor-pointer flex items-center gap-2 whitespace-nowrap ${
                  tripType === option.id
              ...
              >
                <span className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center ${
                  tripType === option.id ? "border-blue-primary bg-blue-primary" : "border-gray-400"
                }`}>
                  {tripType === option.id && <span className="w-1.5 h-1.5 bg-white rounded-full"></span>}
                </span>
                {option.label}
              </button>
            ))}
